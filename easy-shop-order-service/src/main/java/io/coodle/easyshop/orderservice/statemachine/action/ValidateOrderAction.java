...
public class ValidateOrderAction implements Action<OrderState, OrderEvent> {
    ...
    public void execute(StateContext<OrderState, OrderEvent> stateContext) {
        ...
        if (orderOptional.isPresent()) {
            ...
            orderProducer.send(
                    ValidateOrderRequestDto.builder()
                                        .orderDto(orderDto)
                                        .build(),
                OrderTopics.VALIDATE_ORDER_REQUEST);

            log.debug("Order with id {} was sent to topic {}", orderId, OrderTopics.VALIDATE_ORDER_REQUEST);
        } else {
            log.error(
                "Order with id {} couldn't be send to topic {}, cause order not found",
                orderId, OrderTopics.VALIDATE_ORDER_REQUEST
            );
        }
    }
}
