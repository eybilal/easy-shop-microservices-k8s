package io.coodle.easyshop.customerservice.filter;

import com.auth0.jwt.JWT;
import com.auth0.jwt.JWTVerifier;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.interfaces.DecodedJWT;
import io.coodle.easyshop.customerservice.utils.JWTConstants;
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        if ("/refresh-token".equals(httpServletRequest.getServletPath())) {
            ...
            if (authorisation != null && authorisation.startsWith(JWTConstants.TOKEN_PREFIX)) {
                try {
                    String accessToken = authorisation.substring(JWTConstants.TOKEN_PREFIX.length());
